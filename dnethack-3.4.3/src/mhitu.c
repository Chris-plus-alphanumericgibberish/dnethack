...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
	    case AD_HODS:
		 if(uwep){
			...
			if (uwep->opoisoned){
				int viperheads;
				for(viperheads = ((uwep && (uwep->otyp == VIPERWHIP || uwep->otyp == SET_OF_CROW_TALONS)) ? (1+uwep->ostriking) : 1); viperheads; viperheads--){
					Sprintf(buf, "%s %s",
			...
			if (has_blood(youracedata) && !umechanoid
				...
				*/
				if (mtmp->mtame && !mtmp->isminion)
					EDOG(mtmp)->hungrytime += ((int)((youracedata)->cnutrit / 20) + 1);
...
///////////////////////////////////////////////////////////////////////////////////////////
	    case AD_STCK:
		hitmsg(mtmp, mattk);
		if (uncancelled && !u.ustuck && !sticks(youracedata)){
...
//pline("test string!");
			...
			if (dmgtype(youracedata, AD_SEDU)
				...
				pline("%s tries to %s you, but you seem %s.",
				    ...
				    (is_neuter(mtmp->data) || flags.female == mtmp->female) ? "charm" : "seduce",
				    (is_neuter(mtmp->data) || flags.female == mtmp->female) ? "unaffected" : "uninterested");
			    }
			    if(rn2(3)) {
					if (!tele_restrict(mtmp)) (void) rloc(mtmp, FALSE);
					return 3;
...
///////////////////////////////////////////////////////////////////////////////////////////
		...
		if (!mtmp->mcan && !rn2(3)){
		    if (Acid_resistance) {
				...
				exercise(A_STR, FALSE);
				if(!rn2(10)) erode_armor(&youmonst, TRUE);
		    }
		} else dmg = 0;
		break;
///////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////
		...
		if (is_undead(youracedata) || u.sealsActive&SEAL_OSE) {
...
///////////////////////////////////////////////////////////////////////////////////////////
		case AD_SUCK:
			if (noncorporeal(youracedata) || amorphous(youracedata)) dmg = 0;
			else{
...
///////////////////////////////////////////////////////////////////////////////////////////
	    case AD_MALK:
		hitmsg(mtmp, mattk);
		if (!rn2(4) && /*!u.ustuck &&*/ !sticks(youracedata)) {
			if(Upolyd && u.umonnum == PM_TOVE) You("are much too slithy to grab!");
			else u.ustuck = mtmp;
		}
		if (uncancelled) {
...
///////////////////////////////////////////////////////////////////////////////////////////
	...
	if(attacktype_fordmg(youracedata, AT_NONE, AD_STAR)){
		...
		else if(!otmp || otmp != MON_WEP(mtmp)) dmg /= 2;
	}
 ...
 */
	...
	if(dmg) {
		...
		if (Upolyd) {
		    ...
		    lowerlimit = min((int)youracedata->mlevel, u.ulevel);
		} else {
		    hpmax_p = &u.uhpmax;
		    lowerlimit = u.ulevel;
		}
	...
	register struct attack  *mattk;
{
	...
	if (!u.uswallow) {	/* swallows you */
		if (youracedata->msize >= MZ_HUGE) return(0);
		...
		if (touch_petrifies(youracedata) && !resists_ston(mtmp)) {
	...
	switch(mattk->adtyp) {
		...
		case AD_PHYS:
			...
			if (Amphibious && !flaming(youracedata)) tmp = 0;
...
//ifdef SEDUCE
			...
			case AD_SEDU: //duplicate the whole nine yards, just cause
			{
...
register struct obj *obj;
{
	...
	if (dmgtype(youracedata, AD_CORR))
	    is_acid = TRUE;
	else if (dmgtype(youracedata, AD_RUST))
	    is_acid = FALSE;
	else
	    return;
...
register struct attack *mattk;
{
	...
	switch(olduasmon->mattk[i].adtyp) {
	    ...
	    case AD_EACD:
			...
			if (!rn2(3)) erode_armor(mtmp, TRUE);
		...
		break;
		case AD_EDRC:
			pline("%s is splashed by your poison!", Monnam(mtmp));
			if (resists_poison(mtmp)){
				tmp /= 2;
			} else {
				if (!rn2(10)) {
	    ...
	    case AD_ENCH:	/* KMH -- remove enchantment (disenchanter) */
	    	...
	    	return (1);
	    case AD_STAR:
			if((otmp && otmp == MON_WEP(mtmp)) || !hates_silver(mtmp->data)) {
            	tmp = 0;
            } else Your("dress of silver stars sears %s!", mon_nam(mtmp));
			if(otmp){
				if(!otmp->oartifact){
...
//	if (!Upolyd) return 1; //Removed to enable spirit pasive attacks
	...
	if (rn2(3)) switch(youracedata->mattk[i].adtyp) {
	    case AD_PHYS:
	    	if (youracedata->mattk[i].aatyp == AT_BOOM) {
	    	    You("explode!");
